...
RESULTS_CSV = TABLES / "awg_high_fidelity_results.csv"
SUMMARY_MD = DOCS / "AWG_HIGH_FIDELITY_SUMMARY.md"
...
def routing_metrics(fluxes: list[float]) -> dict[str, float]:
    fluxes_np = np.array(fluxes, dtype=float)
    total_flux = float(np.sum(fluxes_np))
    sorted_flux = np.sort(fluxes_np)[::-1]
    dominant_flux = float(sorted_flux[0]) if len(sorted_flux) else 0.0
    second_flux = float(sorted_flux[1]) if len(sorted_flux) > 1 else 0.0
    dominant_fraction = dominant_flux / total_flux if total_flux > 0 else 0.0
    second_to_first = second_flux / dominant_flux if dominant_flux > 0 else 1.0
    return {
        "total_flux": total_flux,
        "dominant_flux": dominant_flux,
        "dominant_fraction": dominant_fraction,
        "second_to_first": second_to_first,
    }


def classify_routing_quality(dominant_fraction: float, second_to_first: float) -> str:
    # A simple project-facing rubric for this simplified FDTD model.
    if dominant_fraction >= 0.60 and second_to_first <= 0.50:
        return "good"
    if dominant_fraction >= 0.40 and second_to_first <= 0.80:
        return "general"
    if dominant_fraction >= 0.30 and second_to_first <= 0.95:
        return "passing"
    return "poor"
